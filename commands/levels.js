...
// Adjust the folder name if needed (if your folder is "levels" in lowercase, use '../levels/levelSystem')
...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (data.badges && data.badges.length > 0) {
      gymBadgeDisplay = data.badges.map((badgeUrl, index) => {
        // Format each badge as a clickable link.
        return `[Badge ${index + 1}](${badgeUrl})`;
      }).join(' | ');
    } else {
    ...
    const embed = createEmbed({
      color: '#0099ff',
      title: `${interaction.user.username}'s Union Circle Level`,
    ...
    // Reply publicly so everyone in the channel sees it.
